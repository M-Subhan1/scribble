        ...
        <nav class="navbar navbar-expand-lg navbar-light border-bottom pb-0">
            <div class="container-fluid">
                <div class="collapse navbar-collapse" id="navbarSupportedContent">
                    ...
                    <button type="button" class="btn btn-primary btn-sm mb-1" id="list_id" value="{{$list->id}}" data-bs-toggle="modal" data-bs-target="#AddColModal">
                        Add Column
                    </button></td> 
                </div>
            </div>
        ...
        <div id = "boxes" class="mr-2"> 
            @foreach($list->list_columns as $column)
                ...
                @foreach($column->list_entries as $entry)
                    <h6>{{$entry->content}}</h6> 
                @endforeach
                <button type="button" value="{{$column->id}}" class="add_bdentry btn btn-primary btn-sm">+ New</button>
                </div>
